...
@neuroglancer.route("/neuroglancer/mkislin/20210715_mk_Ai148_442_2p",
    methods=['GET','POST'])
@logged_in
@log_http_requests
def mkislin_20210715_mk_Ai148_442_2p():
    """ A route for generating a link to Neuroglancer to show Mikhail
    his 2p data. 

    """
    sample_name = "20210715_mk_Ai148_442" 
    username = 'mkislin'
    request_name = 'Ai148_2p_mapping_PNIA_and_PLCb4Titration'
    ...
    cv_container_name = f'{session_name}_2p_{sample_name}'
    cv_name = f'2p_stitched_{sample_name}'
    cv_path = os.path.join(layer_rootdir,'rawdata',f'Ai148-442-Fused')      
    cv_number += 1              
    """ send the data to the viewer-launcher
    to launch the cloudvolume """                       
    cv_dict = dict(cv_path=cv_path,cv_name=cv_name,
    ...
    logger.debug("Made post request to viewer-launcher to launch ng google viewer")
...
@neuroglancer.route("/neuroglancer/mkislin/20210715_mk_Ai148_442_lightsheet",
    methods=['GET','POST'])
@logged_in
@log_http_requests
def mkislin_20210715_mk_Ai148_442_lightsheet():
    """ A route for generating a link to Neuroglancer to show Mikhail
    his 2p data. 
    ...
    # """ CV 1: lightsheet data - Ch 561 """
    layer_type = "image"
               
    cv_container_name = f'{session_name}_lightsheet_{sample_name}_ch561'
    cv_name = f"lightsheet_corrected_{sample_name}_ch561"
    cv_path = os.path.join(layer_rootdir,'rawdata',f'Ex_561_Em_1_corrected')      
    cv_number += 1              
    """ send the data to the viewer-launcher
    to launch the cloudvolume """                       
    cv_dict = dict(cv_path=cv_path,cv_name=cv_name,
    ...
    # """ CV 2: lightsheet data - Ch 642 """
    layer_type = "image"
               
    cv_container_name = f'{session_name}_lightsheet_{sample_name}_ch642'
    cv_name = f"lightsheet_corrected_{sample_name}_ch642"
    cv_path = os.path.join(layer_rootdir,'rawdata',f'Ex_642_Em_2_corrected')      
    cv_number += 1              
    """ send the data to the viewer-launcher
    to launch the cloudvolume """                       
    cv_dict = dict(cv_path=cv_path,cv_name=cv_name,
...
@neuroglancer.route("/neuroglancer/kelly_cfos_smartspim_demo",
    methods=['GET','POST'])
@logged_in
@log_http_requests
def kelly_cfos_smartspim_demo():
    """ A route for generating a link to Neuroglancer to show Chris
    his c-Fos LaVision data for a single brain

    """
    brain_full = '20200901_14_20_11_f37073_mouse1_20171010'
    brain_short = 'f3073_mouse1_ss'
    layer_rootdir = os.path.join('/jukebox/LightSheetData/wang-mouse/seagravesk',
        brain_full,'viz')
    config_proxy_auth_token = os.environ['CONFIGPROXY_AUTH_TOKEN']
    # Redis setup for this session
    kv = redis.Redis(host="redis", decode_responses=True)
    ...
    cv_container_name = f'{session_name}_data_{brain_short}'
    cv_name = f'data_{brain_short}'
    cv_path = os.path.join(layer_rootdir,'corrected',f'channel_785_corrected') 
    print(os.path.exists(cv_path))     
    cv_number += 1              
    """ send the data to the viewer-launcher
    to launch the cloudvolume """                       
    cv_dict = dict(cv_path=cv_path,cv_name=cv_name,
    ...
    cv_container_name = f'{session_name}_rawatlas_{brain_short}'
    cv_name = f"rawatlas_{brain_short}"
    cv_path = os.path.join(layer_rootdir,'raw_atlas',f'{brain_short}_raw_atlas')      
    cv_number += 1              
    """ send the data to the viewer-launcher
    to launch the cloudvolume """                       
    cv_dict = dict(cv_path=cv_path,cv_name=cv_name,
...
@neuroglancer.route("/neuroglancer/kelly_cfos_lavision_demo",
    methods=['GET','POST'])
@logged_in
@log_http_requests
def kelly_cfos_lavision_demo():
    """ A route for generating a link to Neuroglancer to show Chris
    his c-Fos LaVision data for a single brain
    ...
    cv_container_name = f'{session_name}_blended_lavision_{brain_short}'
    cv_name = f'blended_lavision_{brain_short}'
    cv_path = os.path.join(layer_rootdir,'channel790_blended') 
    print(os.path.exists(cv_path))     
    cv_number += 1              
    """ send the data to the viewer-launcher
    to launch the cloudvolume """                       
    ...
    cv_container_name = f'{session_name}_rawatlas_{brain_short}'
    cv_name = f"rawatlas_lavision_{brain_short}"
    cv_path = os.path.join(layer_rootdir,'raw_atlas',)      
    cv_number += 1              
    """ send the data to the viewer-launcher
    to launch the cloudvolume """                       
    cv_dict = dict(cv_path=cv_path,cv_name=cv_name,
    ...
    cv_number += 1              
    cv_container_name = f'{session_name}_rawcells_lavision_{brain_short}'
    cv_name = f"rawcells_lavision_{brain_short}"
...
@neuroglancer.route("/neuroglancer/hboele/HJB_DCN_YFP",
    methods=['GET','POST'])
@logged_in
@log_http_requests
def HJB_DCN_YFP():
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            cv_path = os.path.join(layer_rootdir,'corrected',
                f'{sample_name}_ch642_corrected')
            logger.debug("Layer dir:")
    ...
    return render_template('neuroglancer/lightserv_cfos_setup.html',
        request_name=request_names[0],form=form)
...
@neuroglancer.route("/neuroglancer/zimmerman_04_06_cfos_setup",
    methods=['GET','POST'])
@logged_in
@log_http_requests
def zimmerman_04_06_cfos_setup():
    """ A route for Chris to select which of his available brains
    he wants to make links for in Neuroglancer """
    form = LightservCfosSetupForm(request.form)
    username = 'cz15'
...
def rawdata_launcher():
    ...
    neuroglancerurl = "https://neuroglancer-demo.appspot.com/#!%7B%22dimensions%22:%7B%22x%22:%5B0.000001866%2C%22m%22%5D%2C%22y%22:%5B0.000001866%2C%22m%22%5D%2C%22z%22:%5B0.000002%2C%22m%22%5D%7D%2C%22position%22:%5B3694.601318359375%2C4055.148681640625%2C1858.5001220703125%5D%2C%22crossSectionScale%22:31.343283446669396%2C%22projectionScale%22:16384%2C%22layers%22:%5B%7B%22type%22:%22image%22%2C%22source%22:%22precomputed://https://lightsheetatlas.pni.princeton.edu/public/lightserv/cz15/zimmerman_04/zimmerman_04-355/imaging_request_1/viz/stitched/zimmerman_04-355_ch642_stitched_autocorrected%22%2C%22tab%22:%22rendering%22%2C%22shaderControls%22:%7B%22normalized%22:%7B%22range%22:%5B672%2C0%5D%7D%7D%2C%22name%22:%22zimmerman_04-355_ch642_stitched_autocorrected%22%7D%5D%2C%22selectedLayer%22:%7B%22visible%22:true%2C%22layer%22:%22zimmerman_04-355_ch642_stitched_autocorrected%22%7D%2C%22layout%22:%224panel%22%7D"
    # neuroglancerurl = f"http://{hosturl}/nglancer/{session_name}/v/{viewer_dict['token']}/" # localhost/nglancer is reverse proxied to 8080 inside the ng container
    logger.debug(neuroglancerurl)
...
@neuroglancer.route("/neuroglancer/zimmerman_02_cfos_setup_imaging_request_2",
    methods=['GET','POST'])
@logged_in
@log_http_requests
def zimmerman_02_cfos_setup_imaging_request_2():
    """ A route for Chris to select which of his available brains
    he wants to make links for in Neuroglancer """
    form = LightservCfosSetupForm(request.form)
    username = 'cz15'
...
@neuroglancer.route("/neuroglancer/MO_May2021_Tsc1_viz_setup",
    methods=['GET','POST'])
@logged_in
@log_http_requests
def MO_May2021_Tsc1_viz_setup():
    """ A route for Marlies to select which of her available brains
    she wants to make links for in Neuroglancer """
    form = LightservCfosSetupForm(request.form)
    username = 'oostland'
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            cv_number = 0 # to keep track of how many cloudvolumes in this viewer
            cv_container_name = f'{session_name}_{selected_sample_name}_channel_555'
            ...
            """ CV 2: Raw space atlas (first check that it exists) """

            layer_type = "segmentation"
                       
            cv_container_name = f'{session_name}_{selected_sample_name}_raw_atlas'
            cv_name = f'{selected_sample_name}_raw_atlas'
            cv_path = os.path.join(layer_rootdir,'raw_atlas','raw_atlas')      
            if os.path.exists(cv_path):
                cv_number += 1              
                """ send the data to the viewer-launcher
                to launch the cloudvolume """                       
