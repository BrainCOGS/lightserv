...
@imaging.route("/imaging/imaging_setup/<username>/<request_name>/<imaging_batch_number>",
	methods=['GET','POST'])
@logged_in
@log_http_requests
def imaging_setup(username,request_name,imaging_batch_number): 
	""" Route for deciding whether to image all samples
	in an imaging batch together or individually
	"""
	form = ImagingSetupForm(request.form)
	...
	imaging_batch_table = ImagingBatchTable(imaging_batch_contents)
	imaging_progress = imaging_batch_contents.fetch1('imaging_progress')
	form.sample_dropdown.choices = [(sample_dict['sample_name'],sample_dict['sample_name']) \
		for sample_dict in sample_dict_list]
	if request.method == "POST":
		logger.debug("POST request")
		logger.debug(form.sample_dropdown.data)
		if form.image_together.data == True:
			logger.debug("Image together!")
			return redirect(url_for('imaging.imaging_batch_entry',
				username=username,
				request_name=request_name,imaging_batch_number=imaging_batch_number))
	return render_template('imaging/imaging_setup.html',
		sample_dict_list=sample_dict_list,
		imaging_batch_table=imaging_batch_table,form=form)
...
def imaging_batch_entry(username,request_name,imaging_batch_number): 
	...
	sample_contents = db_lightsheet.Request.Sample() & f'request_name="{request_name}"' & \
			f'username="{username}"' & f'imaging_batch_number="{imaging_batch_number}"' 
	sample_dict_list = sample_contents.fetch(as_dict=True)
	clearing_dict = sample_contents.fetch(
		'clearing_protocol',
		'antibody1','antibody2','clearing_batch_number',limit=1,as_dict=True)[0]
	logger.debug(clearing_dict)
	clearing_protocol = clearing_dict['clearing_protocol']
	antibody1=clearing_dict['antibody1']
	antibody2=clearing_dict['antibody2']
	clearing_batch_number = clearing_dict['clearing_batch_number']
	...
	""" Assemble the imaging subforms, using first sample in batch for lookup """
	...
	channel_contents = (db_lightsheet.Request.ImagingChannel() & f'request_name="{request_name}"' & \
			f'username="{username}"' & f'sample_name="{first_sample_name}"')
	...
	if request.method == 'POST':
		...
		""" Check to see if the "apply batch parameters to all samples" button was pressed"""
		if form.submit.data == False:
			if form.apply_batch_parameters_button.data == True: # The uniform clearing button
				...
				for sample_form in form.sample_forms:
					for ii in range(len(batch_image_resolution_forms)):
						...
						for jj in range(len(batch_channel_forms)):
							...
							sample_channel_form = sample_channel_forms[jj]
							sample_channel_form.zoom_body_magnification.data = batch_channel_form.zoom_body_magnification.data
							sample_channel_form.image_orientation.data = batch_channel_form.image_orientation.data
							sample_channel_form.left_lightsheet_used.data = batch_channel_form.left_lightsheet_used.data
							sample_channel_form.right_lightsheet_used.data = batch_channel_form.right_lightsheet_used.data
							sample_channel_form.tiling_scheme.data = batch_channel_form.tiling_scheme.data
							sample_channel_form.tiling_overlap.data = batch_channel_form.tiling_overlap.data 
							sample_channel_form.z_step.data = batch_channel_form.z_step.data 

							# logger.debug(batch_channel_form.tiling_overlap.data)
							# logger.debug(type(batch_channel_form.tiling_overlap.data))
							# logger.debug(type(sample_channel_form.tiling_overlap.data))
				...
				for form_resolution_dict in form.image_resolution_forms.data:
					...
					if new_channel_button_pressed:
						...
						return redirect(url_for('imaging.imaging_entry',
							username=username,request_name=request_name,sample_name=sample_name,imaging_request_number=imaging_request_number))
		elif form.validate_on_submit(): # final submit button pressed and validated
		...
		for sample_dict in sample_dict_list:
			...
			for ii in range(len(unique_image_resolutions)):
				...
				channel_contents_list_this_resolution = (channel_contents & \
