...
def test_client():
	...
	The way a fixture works is whatever is yielded
...
def test_request_all_mouse_clearing_protocols_ahoag(test_client,
	...
	response = test_client.post(
		url_for('requests.new_request'),data={
			...
			'imaging_samples-1-image_resolution_forms-0-notes_for_imager':'Image horizontally please!',
			...
			'imaging_samples-2-image_resolution_forms-0-notes_for_imager':'Image horizontally please!',
			...
			'imaging_samples-3-image_resolution_forms-0-notes_for_imager':'Image horizontally please!',
...
def test_archival_request_nonadmin(test_client,test_login_nonadmin,test_delete_request_db_contents):
	  ...
	  'time_submitted': '12:55:22', 'labname': 'Wang',
	   'correspondence_email': 'lightserv-test@princeton.edu',
		'description': 'Image c-fos in whole brains at 1.3x.',
		 'species': 'mouse', 'number_of_samples': 1,'raw_data_retention_preference':"important", 'is_archival': True}
	...
	'clearing_batch_number':1, 'imaging_batch_number': 1,
	 'imaging_request_number': 1, 'imaging_progress': 'complete', 
...
def test_request_15x_smartspim_nonadmin(test_client,
	test_login_nonadmin,test_delete_request_db_contents):
	""" Submits a new request as 'lightserv-user' with a single sample requesting 
	to use 15x resolution on the Smartspim
	that can be used for various tests.

	It uses the test_delete_request_db_contents fixture, which means that 
	the entry is deleted as soon as the test has been run
	"""
	print('----------Setup test_request_15x_smartspim_nonadmin fixture ----------')
	with test_client.session_transaction() as sess:
		current_user = sess['user']
	response = test_client.post(
		url_for('requests.new_request'),data={
...
@pytest.fixture(scope='function') 
def test_multisample_multichannel_request_nonadmin(test_client,
	test_login_nonadmin,test_delete_request_db_contents):
	""" Submits a new request as 'lightserv-test' (a nonadmin) that can be used for various tests.
	...
	response = test_client.post(
		url_for('requests.new_request'),data={
			...
			'description':"This is a demo request to submit many samples with many channels each",
			'species':"mouse",'number_of_samples':3,
			'raw_data_retention_preference':"important",
			'username':current_user,
			...
			'imaging_samples-1-image_resolution_forms-0-channel_forms-0-registration':True,
			'imaging_samples-1-image_resolution_forms-0-channel_forms-0-channel_name':'488',
			'imaging_samples-1-image_resolution_forms-0-channel_forms-1-injection_detection':True,
			'imaging_samples-1-image_resolution_forms-0-channel_forms-1-channel_name':'555',
...
@pytest.fixture(scope='function') 
def test_nonadmin_request_with_auditor(test_client,test_login_nonadmin,test_delete_request_db_contents):
	""" Submits a new request as 'lightserv-test' (a nonadmin) with an auditor "audit-user"
	that can be used for various tests.
	...
	response = test_client.post(
		url_for('requests.new_request'),data={
			'labname':"Tank/Brody",'correspondence_email':"lightserv-test@princeton.edu",
			'request_name':"nonadmin_request_with_auditor",
			'description':"This is a request by lightserv-test, a non admin",
			'species':"mouse",'number_of_samples':1,
			'raw_data_retention_preference':"important",
			'username':current_user,
...
@pytest.fixture(scope='function') 
def test_request_both_microscopes_nonadmin(test_client,
	test_login_nonadmin,test_delete_request_db_contents):
	""" Submits a new request as 'lightserv-user' with a single sample requesting 
	to use imaging from both the lavision and smartspim microscopes 
	that can be used for various tests.
	...
	response = test_client.post(
		url_for('requests.new_request'),data={
			'labname':"Wang",'correspondence_email':"test@demo.com",
			'request_name':"nonadmin_bothmicroscopes_request",
			'description':"This is a demo request",
			'species':"mouse",'number_of_samples':1,
			'raw_data_retention_preference':"important",
			'username':test_login_nonadmin['user'],
			...
			'clearing_samples-0-clearing_protocol':'iDISCO abbreviated clearing',
			'clearing_samples-0-sample_name':'sample-001',
			'imaging_samples-0-image_resolution_forms-0-image_resolution':'1.1x',
			'imaging_samples-0-image_resolution_forms-0-notes_for_imager':'Image horizontally please!',
			'imaging_samples-0-image_resolution_forms-0-atlas_name':'allen_2017',
			'imaging_samples-0-image_resolution_forms-0-final_orientation':'sagittal',
			'imaging_samples-0-image_resolution_forsetup':'1.3x',
			'imaging_samples-0-image_resolution_forms-0-channel_forms-0-registration':True,
			'imaging_samples-0-image_resolution_forms-0-channel_forms-0-channel_name':'488',
			'imaging_samples-0-image_resolution_forms-1-image_resolution':'3.6x',
			'imaging_samples-0-image_resolution_forms-1-notes_for_imager':'Image horizontally please!',
			'imaging_samples-0-image_resolution_forms-1-atlas_name':'allen_2017',
			'imaging_samples-0-image_resolution_forms-1-final_orientation':'sagittal',
...
def test_cleared_request_3p6x_smartspim_nonadmin(test_client,
	...
	all db entries are deleted upon teardown of this fixture
	"""
	print('----------Setup test_cleared_request_3p6x_nonadmin fixture ----------')
	now = datetime.now()
	data = dict(time_pbs_wash1=now.strftime('%Y-%m-%dT%H:%M'),
		pbs_wash1_notes='some notes',submit=True)
...
def test_cleared_request_15x_smartspim_nonadmin(test_client,
	...
	""" Clears the single request by 'lightserv-test' (with clearer='ll3')
	where 15x resolution was requested 

	Uses the test_delete_request_db_contents fixture, which means that 
	all db entries are deleted upon teardown of this fixture
	"""
	print('----------Setup test_cleared_request_15_nonadmin fixture ----------')
	now = datetime.now()
	data = dict(time_pbs_wash1=now.strftime('%Y-%m-%dT%H:%M'),
		pbs_wash1_notes='some notes',submit=True)
...
@pytest.fixture(scope='function') 
def test_cleared_multisample_multichannel_request_nonadmin(test_client,
	test_multisample_multichannel_request_nonadmin,test_login_ll3,test_delete_request_db_contents):
	""" Clears the single request by 'lightserv-test' (with clearer='ll3')
	where multiple channels are requested for multiple samples
...
@pytest.fixture(scope='function') 
def test_cleared_request_both_microscopes_nonadmin(test_client,
	test_request_both_microscopes_nonadmin,test_login_ll3,test_delete_request_db_contents):
	""" Clears the single request by 'lightserv-test' (with clearer='ll3')
	where 3p6x resolution was requested 
